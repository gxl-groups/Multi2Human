...
class DalleMaskImageEmbedding(BaseEmbedding):
    def __init__(self,
                 num_embed=18432,
                 spatial_size=[32, 16], # height and with
                 embed_dim=512,
                 trainable=True,
    ...
    def forward(self, index, **kwargs):
        ...
        return emb # torch.Size([1, 512, 1024])


class DalleMask_ImageEmbedding(BaseEmbedding):
    def __init__(self,
                 num_embed=1024,
                 spatial_size=[32, 16],  # height and with
                 embed_dim=512,
    ...
    def forward(self, index, **kwargs):
        ...
        try:
            ...
            raise RuntimeError('IndexError: index out of range in self, max index {}, num embed {}'.format(index.max(),
                                                                                                           self.num_embed))
        ...
        if emb.shape[1] > 0:  #
            if self.pos_emb_type == 'embedding':  #
                height_emb = self.height_emb(
                    torch.arange(self.spatial_size[0], device=index.device).view(1, self.spatial_size[0])).unsqueeze(
                    2)  # 1 x H x D -> 1 x H x 1 x D
                width_emb = self.width_emb(
                    torch.arange(self.spatial_size[1], device=index.device).view(1, self.spatial_size[1])).unsqueeze(
                    1)  # 1 x W x D -> 1 x 1 x W x D
            else:
                height_emb = self.height_emb.unsqueeze(2)  # 1 x H x D -> 1 x H x 1 x D
                width_emb = self.width_emb.unsqueeze(1)  # 1 x W x D -> 1 x 1 x W x D
            pos_emb = (height_emb + width_emb).view(1, self.spatial_size[0] * self.spatial_size[1],
                                                    -1)  # 1 x H x W x D -> 1 x L xD
            emb = emb + pos_emb[:, :emb.shape[1], :]
        return emb  # torch.Size([1, 512, 1024])