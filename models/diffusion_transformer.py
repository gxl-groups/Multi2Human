...
class DiffusionTransformer(nn.Module):
    ...
    def forward(
        ...
        sample_image = img_z.type_as(img_z) # img_token
        ...
        if self.condition_emb is not None:# image_synthesis.modeling.embeddings.clip_text_embedding.CLIPTextEmbedding
            with autocast(enabled=False):
                with torch.no_grad():
                    ...
                    ####
                    cond_emb = self.condition_emb(seg_z) # B x Ld x D  b*77*512
                if self.learnable_cf:
                    is_empty_text = torch.logical_not(seg_z[:, 2]).unsqueeze(1).unsqueeze(2).repeat(1, 77, 512)
