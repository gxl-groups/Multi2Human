...
from .dalle_mask_image_embedding import DalleMaskImageEmbedding
...
class Text2ImageTransformer(nn.Module):
    def __init__(
            ...
            mlp_hidden_times=4,
            attn_type='selfcross',
            content_spatial_size=[32, 16],  # H , W
            condition_dim=512,
            diffusion_step=1000,
    ...
    def forward(
            ...
            cond_emb,
            t):
        # image_synthesis.modeling.embeddings.dalle_mask_image_embedding.DalleMaskImageEmbedding

        cont_emb = self.content_emb(input)

        emb = cont_emb  # torch.Size([1, 512, 1024])

        for block_idx in range(len(self.blocks)):
            if self.use_checkpoint == False:  # cross_attn
                emb, att_weight = self.blocks[block_idx](emb, cond_emb,t.cuda())  # B x (Ld+Lt) x D, B x (Ld+Lt) x (Ld+Lt)
                # print('211',block_idx,att_weight.shape)#  torch.Size([b, 512, 1024])  torch.Size([1, 512, 512])
            else:
                emb, att_weight = checkpoint(self.blocks[block_idx], emb, cond_emb, t.cuda())
        logits = self.to_logits(emb)  # B x (Ld+Lt) x n  # torch.Size([b, 512, 1024])-->  torch.Size([1, 512, 18432])
        out = rearrange(logits, 'b l c -> b c l')  # torch.Size([1,18432,512])
        # print('w',out.shape)
        return out
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        if attn_type in ['selfcross', 'selfcondition', 'self']:  #
            if 'adalayernorm' in timestep_type: #
                self.ln1 = AdaLayerNorm(n_embd, diffusion_step, timestep_type) # img_token&t AdaNm
            else:
    ...
    def forward(self, x, encoder_output, timestep, mask=None):
        # print('attn_type',self.attn_type) selfcross
        if self.attn_type == "selfcross": #
            a, att = self.attn1(self.ln1(x, timestep), encoder_output, mask=mask) # self attn
            x = x + a
            a, att = self.attn2(self.ln1_1(x, timestep), encoder_output, mask=mask) # img*ADaLN做cross attn：img为Q
            x = x + a
