...
def save_images_test(name,images, im_name, step, log_dir, save_individually=False):
    log_dir = "logs/" + log_dir
    os.makedirs(log_dir, exist_ok=True)
    if save_individually:
        for idx in range(len(images)):
        ...
        torchvision.utils.save_image(
            torch.clamp(images, 0, 1),
            f"{log_dir}/{im_name}/{name}",
            nrow=int(np.sqrt(images.shape[0])),
            padding=0
        )

def save_latents(H,name,train_latent_ids):
    save_dir = "latents/"
    ...
    train_latents_fp = f"latents/{name[-1]}"
