...
from models.diffaug import rand_translation
...
# !
    for step in range(start_step, H.train_steps):
        ...
        batch = next(train_iterator) # 2*3*(512,256)

        if isinstance(batch, list): #
            x = batch[0]
            # print('x',x.shape) torch.Size([4, 3, 256, 128])
        else:
            x = batch
        ...
        x = x.cuda() # b*3*(512,256)

        if H.amp: #
            # AMP 与单精度 (FP32) 相比，某些运算在半精度 (FP16) 下运行更快，而不会损失准确率。
            # AMP 会自动决定应该以哪种精度执行哪种运算。这样既可以加快训练速度，又可以减少内存占用。
            optim.zero_grad()
            with torch.cuda.amp.autocast(): # VQGAN重构
                x_hat, stats = vqgan.train_iter(x, step) # stats
            # 混合精度训练 ，效果更好，速度更快，内存占用少。
        ...
        if step > H.disc_start_step: # 预热30k再对D backward(),AMP
            if H.amp: #
                d_optim.zero_grad()
