...
class HparamsVQGAN(HparamsBase):
    def __init__(self, dataset):
        ...
        if self.dataset == "Deepfashion"or self.dataset == "DeepfashionT":
            self.attn_resolutions = [16]
            self.batch_size = 2
            self.ch_mult = [1, 1, 2, 2, 4]
            self.codebook_size = 18432 # 2048 # 原1024
            self.disc_layers = 3
            self.disc_weight_max = 1
            self.disc_start_step = 30001
            self.emb_dim = 256
            self.img_size = 256
            self.latent_shape = [1, 32, 16]
            self.n_channels = 3
            self.ndf = 64
            self.nf = 128
...
def add_vqgan_args(parser):
    ...
    parser.add_argument('--diff_aug', const=True, action='store_const', default=True)
