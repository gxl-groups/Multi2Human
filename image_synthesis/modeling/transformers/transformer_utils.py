...
class Text2ImageTransformer(nn.Module):
    ...
    def forward(
        ...
        cont_emb = self.content_emb(input) #  torch.Size([2, 1024, 1024])
        emb = cont_emb
        ...
        for block_idx in range(len(self.blocks)):   
            if self.use_checkpoint == False:  #
                ...
                # print('211',emb.shape) torch.Size([2, 1024, 1024])
            else:
                emb, att_weight = checkpoint(self.blocks[block_idx], emb, cond_emb, t.cuda())
        logits = self.to_logits(emb) # B x (Ld+Lt) x n  # torch.Size([2, 1024, 1024])--> torch.Size([2, 1024, 2887])
        out = rearrange(logits, 'b l c -> b c l') # torch.Size([2, 2887, 1024])
        return out
