...
def index_to_log_onehot(x, num_classes):
    ...
    x_onehot = F.one_hot(x, num_classes) # torch.Size([2, 1024, 2888])

    permute_order = (0, -1) + tuple(range(1, len(x.size())))
    x_onehot = x_onehot.permute(permute_order) #  torch.Size([2, 2888, 1024])
...
class DiffusionTransformer(nn.Module):
    ...
    def q_pred(self, log_x_start, t):           # q(xt|x0)
        ...
        log_cumprod_at = extract(self.log_cumprod_at, t, log_x_start.shape)         # at~ torch.Size([2, 1, 1])
        log_cumprod_bt = extract(self.log_cumprod_bt, t, log_x_start.shape)         # bt~ torch.Size([2, 1, 1])
        log_cumprod_ct = extract(self.log_cumprod_ct, t, log_x_start.shape)         # ct~ torch.Size([2, 1, 1])
    ...
    def predict_start(self, log_x_t, cond_emb, t):          # p(x0|xt)
        ...
        else: # image_synthesis.modeling.transformers.transformer_utils.Text2ImageTransformer
            out = self.transformer(x_t, cond_emb, t) # torch.Size([2, 2887, 1024])
        ...
        log_pred = torch.cat((log_pred, self.zero_vector), dim=1)  # torch.Size([2, 2888, 1024])
    ...
    def _train_loss(self, x, cond_emb, is_train=True):                       # get the KL loss
        ...
        log_x_start = index_to_log_onehot(x_start, self.num_classes) #  torch.Size([2, 2888, 1024])
        log_xt = self.q_sample(log_x_start=log_x_start, t=t) # xt
        xt = log_onehot_to_index(log_xt) # .argmax(1)

        ############### go to p_theta function ###############
        # P_theta(x0|xt)
        log_x0_recon = self.predict_start(log_xt, cond_emb, t=t)    # torch.Size([2, 2888, 1024])
    ...
    def forward(
        ...
        sample_image = input['content_token'].type_as(input['content_token']) # img_token
        # cont_emb = self.content_emb(sample_image)

        if self.condition_emb is not None:# image_synthesis.modeling.embeddings.clip_text_embedding.CLIPTextEmbedding
            with autocast(enabled=False):
                with torch.no_grad():
                    # input['condition_token'] b*77
                    cond_emb = self.condition_emb(input['condition_token']) # B x Ld x D  b*77*512
        ...
        if is_train == True: #
            log_model_prob, loss = self._train_loss(sample_image, cond_emb) # img_token & text_emb
        ...
        if return_logits:
            ...
            # print('11',out['logits'].shape)  torch.Size([2, 2888, 1024])
