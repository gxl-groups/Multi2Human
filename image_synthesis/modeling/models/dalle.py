...
class DALLE(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.content_info = content_info # {'key': 'image'}
        self.condition_info = condition_info # {'key': 'text'}
        self.guidance_scale = 1.0
        self.learnable_cf = learnable_cf # F
    ...
    def prepare_condition(self, batch, condition=None):
        ...
        # print(1,cond[0],len(cond[0]))
        # text 2batch
        cond = self.condition_codec.get_tokens(cond) # image_synthesis.modeling.codecs.text_codec.tokenize.Tokenize
        # print(2, cond['token'][0],len(cond['token'][0]))
        # token(77len)  mask(77len)
        cond_ = {}
    ...
    def prepare_content(self, batch, with_mask=False):
        ...
        cont = batch[cont_key] # image
        if torch.is_tensor(cont):
            cont = cont.to(self.device)
        if not with_mask: #
            # img tokens
            cont = self.content_codec.get_tokens(cont) # b*1024 1-2887
        else:
        ...
        for k, v in cont.items():
            v = v.to(self.device) if torch.is_tensor(v) else v
            cont_['content_' + k] = v
            # content_token  content_mask
        return cont_

    @autocast(enabled=False)
    @torch.no_grad()
    def prepare_input(self, batch):
        input = self.prepare_condition(batch) # condition_token condition_mask
        input.update(self.prepare_content(batch))
        # condition_token condition_mask content_token
        return input
...
##
    def forward(
        ...
        **kwargs
        ...
        input = self.prepare_input(batch) # dict_keys(['condition_token', 'condition_mask', 'content_token'])
        # image_synthesis.modeling.transformers.diffusion_transformer.DiffusionTransformer
        output = self.transformer(input, **kwargs) # logits torch.Size([2, 2888, 1024]) 、loss
        return output
