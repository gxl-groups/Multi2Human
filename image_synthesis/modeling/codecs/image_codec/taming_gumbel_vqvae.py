...
class Encoder(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x):
        x = 2*x - 1 #  torch.Size([b, 3, 256, 256])
        h = self.encoder(x) #  torch.Size([b, 256, 32, 32])
        h = self.quant_conv(h)#  torch.Size([b, 256, 32, 32])
        quant, _, [_, _, indices] = self.quantize(h) # torch.Size([2, 32, 32])
        # torch.Size([2, 1024])
...
class TamingGumbelVQVAE(BaseCodec):
    def __init__(
        ...
        self.num_tokens = num_tokens
        self.quantize_number = quantize_number
        if self.quantize_number != 0 and mapping_path!=None: # 2887
            self.full_to_quantize = torch.load(mapping_path)
            self.quantize_to_full = torch.zeros(self.quantize_number)-1
            for idx, i in enumerate(self.full_to_quantize):
                if self.quantize_to_full[i] == -1:
    ...
    def get_tokens(self, imgs, **kwargs):
        imgs = self.preprocess(imgs) # div255
        code = self.enc(imgs) # 2*1024  1-16384?
        # print('c1', code)
        if self.quantize_number != 0:#
            code = self.full_to_quantize[code] # 2887 remove useless codebook
            # print('c',code)
        output = {'token': code}
        # output = {'token': rearrange(code, 'b h w -> b (h w)')}
        return output
