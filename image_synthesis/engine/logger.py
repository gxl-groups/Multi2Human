...
class Logger(object):
    def __init__(self, args):
        ...
        if self.is_primary: #
            os.makedirs(self.save_dir, exist_ok=True) # 'OUTPUT/cub200_train'
            
            # save the args and config
            self.config_dir = os.path.join(self.save_dir, 'configs')# 'OUTPUT/cub200_train/configs'
            os.makedirs(self.config_dir, exist_ok=True)
            file_name = os.path.join(self.config_dir, 'args.txt') # 'OUTPUT/cub200_train/configs/args.txt
            write_args(args, file_name)

            log_dir = os.path.join(self.save_dir, 'logs') ## 'OUTPUT/cub200_train/logs'
            if not os.path.exists(log_dir):
                os.makedirs(log_dir, exist_ok=True)
            self.text_writer = open(os.path.join(log_dir, 'log.txt'), 'a') # 'w') #  'OUTPUT/cub200_train/configs/log.txt
            if args.tensorboard: #
    ...
    def save_config(self, config):
        if self.is_primary: #
            save_config_to_yaml(config, os.path.join(self.config_dir, 'config.yaml')) # 'OUTPUT/cub200_train/configs/config.yaml'
