...
from utils.log_utils import (config_log, load_model, log, set_up_visdom, start_training_log,save_images)
...
def main(H, vis):
    ...
    generator = Generator(H) # Transformer
    ...
    step = 1
    time_steps = shape[1] * shape[2] # 32*16
    ...
    with torch.no_grad():
        ...
        ).permute(0, 3, 1, 2).contiguous() #  torch.Size([8, 256, 32, 16])
        all_images = []
        del model
        for image_latents in torch.split(q, 8):
            all_images.append(generator(image_latents))

        gen_images = torch.cat(all_images, dim=0) #  torch.Size([8, 3, 512, 256])
